...
def main():
    ...
    model = PPO(
        "MultiInputPolicy" if isinstance(env.observation_space, spaces.Dict) else "MlpPolicy",
        env,
        verbose=0,
        seed=args.seed,
        n_steps=512,
        ...
        learning_rate=3e-4,
        clip_range=0.2,
        ent_coef=0.02,  # slightly higher entropy for exploration with small action space
    )
